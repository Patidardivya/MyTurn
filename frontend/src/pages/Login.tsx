  ...
  return (
    <main className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <section className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        ...
        <h1 className="text-3xl font-bold text-slate-900 mt-8">Welcome back</h1>
        <p className="text-slate-500 mt-2">Log in to manage your tokens and appointments.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
          ...
          <label className="block text-sm font-medium text-slate-700">
            ...
            <input required minLength={6} type="password" value={password} onChange={event => setPassword(event.target.value)}
              className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2.5" autoComplete="current-password" />
          </label>
        ...
        <p className="mt-6 text-center text-sm text-slate-600">New to MyTurn? <Link className="font-semibold text-indigo-600" to="/register">Create an account</Link></p>
      </section>
    </main>
  );
};
